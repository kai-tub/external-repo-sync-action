...
- [External repository sync action](#external-repository-sync-action)
  - [When to use this action](#when-to-use-this-action)
    - [Automatically pushing to the repos wiki](#automatically-pushing-to-the-repos-wiki)
...
1. [Automatically push to the repos wiki](#automatically-pushing-to-the-repos-wiki) (default setting)
2. [Update and push a different repository](#updating-and-pushing-to-a-different-repository)

### Automatically pushing to the repos wiki
...
an hosting solution for automatically generated files/binaries.

This is useful for repositories that need/want to display
the newest graphical changes or example images without
increasing the size of the repository. Or to simply keep the
wiki up to date.

### Updating and pushing to a different repository

Instead of pushing specified files/binaries to the wiki of the current
repository, one can also specify a separate target repository.
This may come in handy if there exists a separate developer/nighlty repository
and a stable/production repository.

## When not to use the action

If the goal is to modify and push to the exisiting repository, a different action
...
0. Enable your repository's wikis feature
    1. If you want to push to the wiki, the wiki has to be initialized first.
...
1. To use this action a Personal Access Token has to be used
...
As described in the set-up description, to be able to use the action
a personal github access token as to be generated and added to the secrets
of the repo and passed in as an environment variable.
...
By default the wiki of the current repository will be used.
- `user`: Set the user name configuration for the push. Default name is 
...
It will never create a new one. Default is master.
...
A couple different examples on how to use the action:
...
At the end of the day the action is an *rsync* and *git* wrapper with some
sensible settings. The action will only clone the specified branch
...
but I decided to *not* add the feature, as NTFS filesystems are
case insensitive.

## Security concerns/Implications

To ensure your secrets cannot be misused by
others, they are not passed into workflows triggered
...
As a consequence it is not straight-forward to automatically
push to the other repo or the wiki when merging branches from forks.
I suggest triggering on deploy events or, for a very hacky solution,
